...
public class Test {
    ...
    static String dir = "C:/Users/xiaodao/Desktop/book/";//文件所在路径，所有文件的根目录，记得修改为你电脑上的文件所在路径
            ...
            if (null == fileArr || fileArr.length == 0) {
                ...
                for (File file : fileArr) {
                    if (file.isDirectory()) {//是文件夹，继续递归，如果需要重命名文件夹，这里可以做处理
                        ...
                        if (fileName.contains("@java1234.com")) {//文件名包含需要被替换的字符串
                            newName = fileName.replaceAll("@java1234.com", "");//新名字
                            newDir = new File(parentPath + "/" + newName);//文件所在文件夹路径+新文件名
                            file.renameTo(newDir);//重命名
                            System.out.println("修改后：" + newDir);
                        }

                       if (fileName.contains("@www.java1234.com")) {//文件名包含需要被替换的字符串
                            newName = fileName.replaceAll("@www.java1234.com", "");//新名字
                            newDir = new File(parentPath + "/" + newName);//文件所在文件夹路径+新文件名
                            file.renameTo(newDir);//重命名
                            System.out.println("修改后：" + newDir);
                        }
